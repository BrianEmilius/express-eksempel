...
export default async function getAllCheeses(request, response) {
	...
	const nextLink = skip + limit >= length ? null : process.env.HOST_ADDRESS + `/api/v1/cheeses?limit=${limit}&skip=${skip + limit}`
	const previousLink = skip === 0
		? null
		: process.env.HOST_ADDRESS
			+ `/api/v1/cheeses?limit=${limit}&skip=${skip - limit < 0 ? 0 : skip - limit}`

	const presentation = {
		count: length,
		next: nextLink,
		previous: previousLink,
		results: result.map(item => ({...item, url: URLBuilder(item._id, "cheese")}))
	}

	response.json(id ? {...result[0], url: URLBuilder(result[0]._id, "cheese")} : presentation)
	response.end()
}
